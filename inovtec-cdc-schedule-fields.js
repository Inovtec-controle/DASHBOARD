...
const TYPES={jours:"Selon jours",quotidien:"Quotidien",hebdomadaire:"Hebdomadaire",bimensuel:"Bimensuel",mensuel:"Mensuel",trimestriel:"Trimestriel",semestriel:"Semestriel",biannuel:"Bi-annuel",annuel:"Annuel",autre:"Autre"};
...
function inferType(row){
 ...
 if(row?.frequenceType==="pair_impair"||row?.frequenceType==="ponctuel")return"autre";
...
function ensureFields(d){const form=d.getElementById("ivCdcForm"),overlay=d.getElementById("ivCdcOverlay");if(!form||!overlay)return;ensureStyle(d);if(!d.getElementById("ivCdcFrequenceType")){const old=d.getElementById("ivCdcFrequence"),oldField=old?.closest(".field");if(oldField){const lab=oldField.querySelector("label");if(lab)lab.textContent="Détail / fréquence libre"}const typeField=d.createElement("div");typeField.className="field";typeField.innerHTML='<label for="ivCdcFrequenceType">Type de fréquence</label><select id="ivCdcFrequenceType"><option value="jours">Selon jours</option><option value="quotidien">Quotidien</option><option value="hebdomadaire">Hebdomadaire</option><option value="bimensuel">Bimensuel</option><option value="mensuel">Mensuel</option><option value="trimestriel">Trimestriel</option><option value="semestriel">Semestriel</option><option value="biannuel">Bi-annuel</option><option value="annuel">Annuel</option><option value="autre">Autre</option></select>';if(oldField)oldField.insertAdjacentElement("afterend",typeField);else form.querySelector(".iv-cdc-form-grid")?.prepend(typeField);const days=d.createElement("div");days.className="field iv-cdc-days-field";days.id="ivCdcDaysField";days.innerHTML='<label>Jours prévus</label><div class="iv-cdc-days">'+DAYS.map(([k,l])=>`<label class="iv-cdc-day"><input type="checkbox" data-iv-cdc-day="${k}"> ${l}</label>`).join("")+'</div><div class="iv-cdc-structured-note"><strong>Données structurées :</strong> ces jours peuvent être réutilisés dans le Planning et les imports Excel / PDF.</div>';typeField.insertAdjacentElement("afterend",days)}if(form.dataset.ivStructuredSave!=="1"){form.dataset.ivStructuredSave="1";form.addEventListener("submit",e=>{if(form.dataset.ivOperationalFix==="1"&&typeof window.InovtecCdcOperationalSave==="function")return;e.preventDefault();e.stopImmediatePropagation();saveStructured(d)},true)}}
function populateStructuredFields(d,row){
 ensureFields(d);
 const type=d.getElementById("ivCdcFrequenceType");if(type)type.value=inferType(row||{});
 const selected=new Set(Array.isArray(row?.jours)?row.jours.map(norm):[]);
...
window.InovtecCahierDesChargesSchema={populateEditor:populateStructuredFields,version:2,fields:["zone","prestation","frequenceType","frequence","jours","methodeConsigne","controle","observations","ordre","sourceType"],days:DAYS.map(x=>x[0]),frequencyTypes:Object.keys(TYPES),supportedSources:["manual","excel","pdf"]};
