...
let siteId="",siteName="",pending=null,resolving=null,readVersion=0,committing=false;
const norm=v=>String(v??"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim(),txt=v=>String(v??"").replace(/\s+/g," ").trim(),rowUid=()=>"cdc_"+Date.now()+"_"+Math.random().toString(16).slice(2);
const freq=(f,j=[])=>{
 f=norm(f);
 if(/semaine.*paire/.test(f)&&/impaire/.test(f))return"pair_impair";
 if(/bimensuel|(?:deux|2) fois (?:par )?mois/.test(f))return"bimensuel";
 if(/trimestriel|chaque trimestre|(?:tous les |tous )?3 mois/.test(f))return"trimestriel";
 if(/semestriel|chaque semestre|(?:tous les |tous )?6 mois/.test(f))return"semestriel";
 if(/bi annuel|biannuel|(?:deux|2) fois (?:par )?an/.test(f))return"biannuel";
 if(/annuel|chaque an|(?:une|1) fois (?:par )?an/.test(f))return"annuel";
 if(/mensuel|chaque mois|(?:une|1) fois (?:par )?mois/.test(f))return"mensuel";
 if(/quotidien|tous les jours|chaque jour/.test(f))return"quotidien";
 if(/hebdo|chaque semaine|(?:une|1) fois (?:par )?semaine/.test(f))return"hebdomadaire";
 return j.length?"jours":f?"autre":"jours";
};
...
function prepareRows(result){
 // PDF text may wrap across rows. Excel already provides the real cell boundaries.
 const rebuilt=result.sourceType==="pdf"?rebuildWrappedRows(result.rows||[]):{rows:result.rows||[],merged:0};
 const rows=[],seen=new Set();let duplicates=0;
 for(const value of rebuilt.rows){const row=clean(value),k=key(row);if(!row.prestation)continue;if(seen.has(k)){duplicates++;continue}seen.add(k);rows.push(row)}
 return{rows,merged:rebuilt.merged,duplicates};
}
...
async function read(d,file){
 ...
 const attempt=++readVersion,target=await resolveSite(d,true);
 if(attempt!==readVersion)return;
 ...
 d.getElementById("ivImpP").innerHTML="";d.getElementById("ivImpGo").disabled=true;
 ...
 try{
  ...
  const P=window.InovtecCdcImportParsers||{},res=ext==="pdf"?await P.pdf?.(file):await P.excel?.(file);
  if(attempt!==readVersion)return;
  if(!res)throw Error("Le module de lecture du fichier n’est pas disponible.");
  const rebuilt=prepareRows(res),out=rebuilt.rows;
  if(!out.length)throw Error(res.reason||"Aucune prestation exploitable détectée.");
  if(out.length>600)throw Error("Import limité à 600 prestations par fichier.");
  const importWarnings=[...(res.warnings||[])];
  if(rebuilt.merged)importWarnings.push(`${rebuilt.merged} morceau${rebuilt.merged>1?"x":""} de texte regroupé${rebuilt.merged>1?"s":""} en prestation${rebuilt.merged>1?"s":""} complète${rebuilt.merged>1?"s":""}.`);
  if(rebuilt.duplicates)importWarnings.push(`${rebuilt.duplicates} doublon${rebuilt.duplicates>1?"s":""} identique${rebuilt.duplicates>1?"s":""} ignoré${rebuilt.duplicates>1?"s":""}.`);
  pending={rows:out,sourceType:res.sourceType,fileName:file.name,fileType:file.type||"",warnings:importWarnings,targetSiteId:target.id,targetSiteName:target.nom||siteName};
  d.getElementById("ivImpSub").textContent=`${file.name} · aperçu avant import dans ${pending.targetSiteName||"ce chantier"}`;
  status(d,`${out.length} prestation${out.length>1?"s":""} détectée${out.length>1?"s":""}. Vérifie puis confirme.`,"ok");preview(d);
  if(pending.warnings.length){const n=d.createElement("div");n.className="ivImpNote";n.textContent=pending.warnings.join(" · ");d.getElementById("ivImpP").appendChild(n)}
 }catch(e){if(attempt!==readVersion)return;console.error(e);status(d,e.message||"Impossible d’analyser ce fichier.","err")}
}
async function commit(d){if(!pending||committing)return;const batch=pending;const targetId=String(batch.targetSiteId||siteId||"");if(!targetId){status(d,"Chantier introuvable. Ferme l’import, resélectionne le chantier et recommence.","err");return}const go=d.getElementById("ivImpGo"),mode=d.getElementById("ivImpMode").value,now=Date.now(),imp=`import_${now}_${Math.random().toString(16).slice(2)}`;committing=true;go.disabled=true;["ivImpX","ivImpCancel","ivImpMode"].forEach(id=>d.getElementById(id).disabled=true);status(d,"Enregistrement dans Firebase…");try{await db.runTransaction(async tx=>{const ref=db.collection("chantiers").doc(targetId),snap=await tx.get(ref);if(!snap.exists)throw Error("Chantier introuvable");const old=snap.data()?.cahierDesChargesV1?.rows||[];let base=mode==="replace_all"?[]:mode==="replace_imported"?old.filter(x=>!["excel","pdf"].includes(x.sourceType)):old.map(x=>({...x}));const seen=new Set(base.map(key));let ord=Math.max(0,...base.map(x=>+x.ordre||0)),added=[];for(const r of batch.rows){const k=key(r);if(mode==="append"&&seen.has(k))continue;seen.add(k);ord+=10;added.push({id:rowUid(),...r,ordre:ord,sourceType:batch.sourceType,sourceFileName:batch.fileName,sourceFileType:batch.fileType,importId:imp,importedAtMs:now,createdAtMs:now,updatedAtMs:now})}const next=base.concat(added);if(JSON.stringify(next).length>800000)throw Error("Cahier des charges trop volumineux pour ce chantier.");tx.update(ref,{"cahierDesChargesV1.schemaVersion":2,"cahierDesChargesV1.structuredSchedule":true,"cahierDesChargesV1.rows":next,"cahierDesChargesV1.updatedAtMs":now,"cahierDesChargesV1.updatedBy":auth?.currentUser?.uid||"","cahierDesChargesV1.lastImport":{id:imp,sourceType:batch.sourceType,fileName:batch.fileName,rowCount:added.length,importedAtMs:now}})});status(d,`Import terminé avec succès dans ${batch.targetSiteName||siteName||"ce chantier"}.`,"ok");go.textContent="Import terminé ✓";setTimeout(()=>{d.getElementById("ivCdcImp").hidden=true;go.textContent="Importer dans ce chantier";pending=null;committing=false;["ivImpX","ivImpCancel","ivImpMode"].forEach(id=>d.getElementById(id).disabled=false)},1200)}catch(e){console.error(e);status(d,e.message||"Import impossible.","err");go.disabled=false;committing=false;["ivImpX","ivImpCancel","ivImpMode"].forEach(id=>d.getElementById(id).disabled=false)}}
...
window.InovtecCdcImport={freq,clean,rebuildWrappedRows,prepareRows,resolveSite:()=>{const d=D();return d?resolveSite(d,true):Promise.resolve(null)}};if(frame){frame.addEventListener("load",()=>setTimeout(install,180));setInterval(install,650);setTimeout(install,450)}try{window.InovtecDataHub?.subscribe?.(install)}catch{}
})();
