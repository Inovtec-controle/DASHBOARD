...
const mode=(new URLSearchParams(location.search).get("mode")||"planning").toLowerCase();
let timer=null,lastHtml="";
const txt=(el,fallback="—")=>{const v=(el?.textContent||el?.value||"").trim();return v||fallback};
function doc(){try{return frame.contentDocument||null}catch{return null}}
function kontrolDoc(){try{return doc()?.getElementById("kontrolFrame")?.contentDocument||null}catch{return null}}
function inject(d,m){if(!d?.head||!d.body)return;d.body.classList.add("iv-visual-v2",`iv-mode-${m}`);if(!d.querySelector('link[data-inovtec-v2="1"]')){const l=d.createElement("link");l.rel="stylesheet";l.href="inovtec-visual-v2.css?v=20260817-orga-final";l.dataset.inovtecV2="1";d.head.appendChild(l)}}
function card(icon,label,value,note="",tone=""){return `<article class="iv-summary-card ${tone}"><span class="iv-summary-icon">${icon}</span><span class="iv-summary-copy"><small class="iv-summary-label">${label}</small><strong class="iv-summary-value">${value}</strong><span class="iv-summary-note">${note}</span></span></article>`}
function countText(root,selector){return root?.querySelectorAll(selector)?.length||0}
function planning(d){const title=txt(d.getElementById("title"),"Planning").replace(/^Planning\s*[—-]?\s*/i,"")||"Agent";const week=txt(d.getElementById("weekInfo"));const count=txt(d.getElementById("count"),"0 intervention");let total=0;[...d.querySelectorAll("#rows tr")].forEach(tr=>{const s=tr.cells?.[1]?.textContent||"";const m=s.match(/(\d{2}):(\d{2})\s*[–-]\s*(\d{2}):(\d{2})/);if(m)total+=Math.max(0,(+m[3]*60 + +m[4])-(+m[1]*60 + +m[2]));});const hm=total?`${Math.floor(total/60)} h ${String(total%60).padStart(2,"0")}`:"0 h";return [card("♙","Agent actif",title,"Référentiel partagé"),card("▦","Semaine",week,"Période affichée"),card("✓","Interventions",count,"Planning sélectionné"),card("◷","Volume prévu",hm,"Calcul des horaires")];}
function discipline(d){const all=countText(d,"#recordsWrap tbody tr")||countText(d,"#recordsWrap .record");const rows=[...d.querySelectorAll("#recordsWrap tr")];const open=rows.filter(r=>/ouvert|en cours/i.test(r.textContent||"")).length;const agent=txt(d.getElementById("agent"),"Aucun agent");return [card("⚑","Dossiers",String(all),"Historique visible"),card("!","À suivre",String(open),open?"Ouverts / en cours":"Aucun dossier actif",open?"warning":""),card("♙","Agent du dossier",agent,"Lié au référentiel")];}
function infoAssignedAgents(d){
 const form=d.getElementById("siteForm"),hub=window.InovtecDataHub,agents=Array.from(hub?.agents||[]);
 let ids=[];
 try{const parsed=JSON.parse(String(form?.dataset.ivAgentsAffectes||""));if(Array.isArray(parsed))ids=[...new Set(parsed.map(v=>String(v||"").trim()).filter(Boolean))]}catch{}
 if(!ids.length){const siteId=String(form?.dataset.ivChantierId||"").trim(),site=Array.from(hub?.chantiers||[]).find(c=>String(c.id)===siteId);if(Array.isArray(site?.agentsAffectes))ids=[...new Set(site.agentsAffectes.map(v=>String(v||"").trim()).filter(Boolean))]}
 if(ids.length){const names=ids.map(id=>{const a=agents.find(x=>String(x.id)===id),i=a?.identity||{};return[i.prenom,i.nom].filter(Boolean).join(" ").trim()||a?.displayName||a?.name||"Agent"});return names.length<=2?names.join(", "):names[0]+" + "+(names.length-1)+" autre"+(names.length>2?"s":"")}
 return txt(d.getElementById("agentNom"),"Non affecté");
}
function infos(d){const count=txt(d.getElementById("count"),"0");const site=txt(d.getElementById("nom"),"Aucun chantier");const agent=infoAssignedAgents(d);return [card("▣","Chantiers",count,"Classés alphabétiquement"),card("⌖","Chantier actif",site,"Fiche sélectionnée"),card("♙","Agent affecté",agent,"Référentiel commun")];}
function agents(d){const count=txt(d.getElementById("agentCount"),"0");const selected=txt(d.getElementById("selectedChip"),"Aucun");const docs=txt(d.getElementById("docCount"),"0 fichier");const inc=txt(d.getElementById("incCount"),"0 entrée");return [card("♙","Agents",count,"Classeur central"),card("◎","Sélection",selected,"Contexte actif"),card("▤","Documents",docs,"Pièces liées"),card("⚑","Incidents",inc,"Suivi agent")];}
function organisation(d){const tasks=countText(d,"#board .task");const overdue=[...d.querySelectorAll("#board .task")].filter(x=>/retard|échéance/i.test(x.textContent||"")&&x.querySelector(".overdue")).length;const archives=txt(d.getElementById("archiveCount"),"0");return [card("◎","Tâches actives",String(tasks),"Tableau opérationnel"),card("!","En retard",String(overdue),overdue?"À traiter":"Aucun retard",overdue?"warning":""),card("▤","Archives",archives,"Tâches terminées")];}
