...
function drawGrid(doc,data){
  const x=8,y=37,w=281,headH=12,timeW=14,bodyH=144,hourH=bodyH/(END_HOUR-START_HOUR),dayW=(w-timeW)/7,bodyY=y+headH;
  doc.setFillColor(243,247,245);doc.rect(x,y,w,headH,"F");doc.setDrawColor(178,178,178);doc.setLineWidth(.22);doc.rect(x,y,w,headH+bodyH);doc.line(x+timeW,y,x+timeW,y+headH+bodyH);
  for(let i=1;i<7;i++){const vx=x+timeW+i*dayW;doc.line(vx,y,vx,y+headH+bodyH)}
  doc.setFont("helvetica","bold");doc.setFontSize(7);doc.setTextColor(25,73,57);
  DAYS.forEach((name,i)=>{const d=addDays(data.start,i),cx=x+timeW+i*dayW+dayW/2;doc.text(name,cx,y+4.8,{align:"center"});doc.setFont("helvetica","normal");doc.setFontSize(6);doc.setTextColor(100,112,106);doc.text(`${d.getDate()} ${MONTHS[d.getMonth()].slice(0,4)}.`,cx,y+9,{align:"center"});doc.setFont("helvetica","bold");doc.setFontSize(7);doc.setTextColor(25,73,57)});
  doc.setFont("helvetica","normal");
  for(let h=START_HOUR;h<=END_HOUR;h++){const yy=bodyY+(h-START_HOUR)*hourH;doc.setDrawColor(178,178,178);doc.setLineWidth(.2);doc.line(x,yy,x+w,yy);if(h<END_HOUR){const hy=yy+hourH/2;doc.setDrawColor(214,214,214);doc.setLineWidth(.12);doc.line(x+timeW,hy,x+w,hy)}doc.setFontSize(6);doc.setTextColor(85,94,89);doc.text(`${pad(h)}:00`,x+timeW-1.4,yy+1.8,{align:"right"})}
  data.groups.forEach((list,day)=>{list.forEach(item=>{
    const s=Math.max(START_HOUR*60,timeToMin(item.start)),e=Math.min(END_HOUR*60,Math.max(s+15,timeToMin(item.end)));if(e<=START_HOUR*60||s>=END_HOUR*60)return;
    const ex=x+timeW+day*dayW+1,ey=bodyY+((s-START_HOUR*60)/60)*hourH,eh=Math.max(4.8,((e-s)/60)*hourH),ew=dayW-2,textW=ew-3;
    doc.setFillColor(232,245,238);doc.setDrawColor(76,156,114);doc.setLineWidth(.25);doc.roundedRect(ex,ey,ew,eh,1.2,1.2,"FD");doc.setFillColor(8,118,84);doc.rect(ex,ey,1,eh,"F");
    if(eh>=8){
      doc.setTextColor(7,93,67);doc.setFont("helvetica","bold");doc.setFontSize(5.2);doc.text(fitLine(doc,`${item.start} - ${item.end}`,textW),ex+2,ey+2.2);
      doc.setTextColor(23,59,48);doc.setFontSize(5.5);doc.text(fitLine(doc,item.task||"Intervention",textW),ex+2,ey+4.8);
      if(item.brief){doc.setFont("helvetica","normal");doc.setTextColor(82,102,93);doc.setFontSize(4.7);doc.text(fitLine(doc,item.brief,textW),ex+2,Math.min(ey+7.1,ey+eh-.45));}
    }else{
      doc.setTextColor(7,93,67);doc.setFont("helvetica","bold");doc.setFontSize(4.1);doc.text(fitLine(doc,`${item.start}-${item.end}`,textW),ex+2,ey+1.35);
      doc.setTextColor(23,59,48);doc.setFontSize(4.3);doc.text(fitLine(doc,item.task||"Intervention",textW),ex+2,ey+2.95);
      if(item.brief){doc.setFont("helvetica","normal");doc.setTextColor(82,102,93);doc.setFontSize(3.7);doc.text(fitLine(doc,item.brief,textW),ex+2,Math.min(ey+4.35,ey+eh-.25));}
    }
    doc.setFont("helvetica","normal");
  })});
