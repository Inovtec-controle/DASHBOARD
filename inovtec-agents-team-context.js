/* Pont ciblé Classeur agents -> éditeur de planning d'équipe sécurisé.
   Ne touche pas aux données métier ; l'ancien moteur d'autosynchronisation
   reste désactivé par planning-protect-existing.js. */
(()=>{
'use strict';
...
const REQUEST='ivAgentsOpenTeamEditorV1',PLANNING='inovtec_plannings_v2';
...
if(mode==='agents'){
 ...
 function install(){
  ...
  list.addEventListener('contextmenu',e=>{
   ...
   button.onclick=()=>{
    close();
    try{sessionStorage.setItem(REQUEST,JSON.stringify({agentId,createdAt:Date.now()}))}
...
function ensureSafeEditor(){
 ...
 script.dataset.ivSafeTeamEditor='1';
...
function openRequested(){
 ...
 const d=doc(),s=read(PLANNING,{});
 const a=Array.isArray(s.agents)?s.agents.find(x=>str(x?.id)===str(req.agentId)||str(x?.refId)===str(req.agentId)):null;
 if(a&&d?.defaultView?.InovtecSafeTeamPlanning?.open(a.id)){
  clearRequest();return;
 }
 if(++attempts>=50){
  clearRequest();
...
frame?.addEventListener('load',()=>{attempts=0;setTimeout(openRequested,80)});
const check=setInterval(()=>{if(!request()){ensureSafeEditor();clearInterval(check);return}openRequested()},350);
setTimeout(openRequested,450);
})();
