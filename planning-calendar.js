...
let currentDate=new Date(),view="week",visibleAgents=new Set(),selectedEvent=null,suppressClickUntil=0,hubBound=false;
...
function createOnDoubleClick(ev){if(ev.button!==0||ev.target.closest(".event-card"))return;if(!state.selected){alert("Sélectionne d’abord un agent dans la liste de gauche.");return}ev.preventDefault();closeEditor();closeContext();const col=ev.currentTarget,start=minuteFromPointer(col,ev.clientY),end=Math.min(END_HOUR*60,start+60),week=col.dataset.week,day=Number(col.dataset.day),event={id:uid("e"),agentId:state.selected,day,start:minToTime(start),end:minToTime(end),task:"Nouvelle intervention",site:"",chantierId:"",note:""};entriesForWeek(week).push(event);markParityEdited(week,state.selected);save();selectedEvent=event.id;render();setTimeout(()=>openEditor(week,event,ev.clientX,ev.clientY),20)}
...
function createDefaultForDate(d,x,y){if(!state.selected){alert("Sélectionne d’abord un agent.");return}const start=9*60,event={id:uid("e"),agentId:state.selected,day:mondayIndex(d),start:minToTime(start),end:minToTime(start+60),task:"Nouvelle intervention",site:"",chantierId:"",note:""},week=isoWeekKey(d);entriesForWeek(week).push(event);markParityEdited(week,state.selected);save();render();setTimeout(()=>openEditor(week,event,x,y),20)}
