...
function bind(){const d=D(),w=W();if(!d?.body||!w)return;ensureLayoutSelector(d,w);if(w.__ivAgentPdfDirectV8)return;w.__ivAgentPdfDirectV8=true;w.addEventListener("click",e=>{const b=e.target?.closest?.("#pdfBtn");if(!b)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();generate(d,w,b)},true);const b=d.getElementById("pdfBtn");if(b){b.title="Generer la fiche agent PDF selon la mise en page choisie";b.dataset.ivPdfDirect="10"}}
