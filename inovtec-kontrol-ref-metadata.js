...
async function sync(){if(!user||busy)return;busy=true;try{const [workspace,allSites,metaSnap]=await Promise.all([db.collection("chantiers").doc(SHARED_ID).get(),db.collection("chantiers").get(),db.collection("chantiers").where("_type","==","kontrolPdfMeta").get()]),w=workspace.exists?(workspace.data()||{}):{},agents=agentsFromWorkspace(w),sites=allSites.docs.map(d=>({id:d.id,...(d.data()||{})})).filter(c=>!c._hidden),updates=[];for(const doc of metaSnap.docs){
  const data=doc.data()||{},custom=data.customMetadata||{},directId=String(data.chantierId||custom.chantierId||"").trim();
  const site=directId?sites.find(c=>String(c.id)===directId)||null:resolveSite(custom.site||data.site||"",sites);
  const ids=resolveAgents(custom.agents||data.agents||"",agents),next={referenceVersion:2};
  if(site){
    next.chantierId=String(site.id);
    next.customMetadata={...custom,chantierId:String(site.id)};
  }
  if(ids.length)next.agentRefIds=[...new Set(ids)];
  const needsMeta=(site&&String(data.chantierId||custom.chantierId||"")!==String(site.id))||(ids.length&&JSON.stringify(data.agentRefIds||[])!==JSON.stringify(next.agentRefIds))||Number(data.referenceVersion)!==2;
  if(needsMeta)updates.push(doc.ref.set(next,{merge:true}));
  if(site){
    updates.push(db.collection("chantiers").doc(String(site.id)).set({
      kontrolHistoryMetaIds:firebase.firestore.FieldValue.arrayUnion(doc.id),
      kontrolHistoryUpdatedAtMs:Date.now()
    },{merge:true}));
  }
}
await Promise.all(updates)}catch(e){console.warn("Références KONTROL",e)}finally{busy=false}}
