...
const configs={
 planning:{label:"Planning",eyebrow:"PLANNING",title:'Planning des <em>équipes</em>',subtitle:"Organisez et suivez les interventions de vos agents sur l’ensemble de vos chantiers.",route:"PLANNINGS.html?v=20260817-containers1",icon:"▦"},
 kontrol:{label:"KONTROL",eyebrow:"CONTRÔLES QUALITÉ",title:"KONTROL",subtitle:"Réalisez vos contrôles qualité sans modifier les critères, réponses, calculs ni archives déjà en place.",route:"KONTROL-CLOUD.html",icon:"✓"},
 ...
 infos:{label:"Infos chantier",eyebrow:"INFOS CHANTIER",title:"Informations <em>chantiers</em>",subtitle:"Retrouvez les accès, contacts, plannings et consignes de chaque résidence dans une présentation plus lisible.",route:"INFOCHANTIERS-V2.html?v=20260817-containers1",icon:"ⓘ"},
 ...
 heures:{label:"Heures sup.",eyebrow:"GESTION DU TEMPS",title:"Heures <em>supplémentaires</em>",subtitle:"Saisissez et analysez les heures supplémentaires avec les calculs et données existants.",route:"HEURE-SUP.html",icon:"＋"},
...
const nav=[configs.planning,configs.kontrol,configs.discipline,configs.infos,configs.agents,configs.organisation,configs.heures,configs.temps,configs.salaire];
...
function buildTools(){tools.innerHTML="";
 ...
 if(mode==="heures"){addTool("Ajouter / déclarer","＋",()=>clickTarget(null,["ajouter","déclarer"]),true);addTool("Exporter","⇩",()=>clickTarget(null,["exporter"]));}
...
function mirrorStatus(){const doc=targetDoc();if(!doc)return;const src=doc.querySelector("#syncStatus,.status.ok,.status.warning");if(src&&src.textContent.trim())$("syncMirror").textContent=src.textContent.trim().slice(0,70);const live=$("liveMirror");if(!live)return;let count="";if(mode==="planning")count=doc.querySelector("#count")?.textContent||"";else if(mode==="infos")count=(doc.querySelector("#count")?.textContent||"")+" chantier(s)";else if(mode==="discipline")count=doc.querySelector("#count")?.textContent||"";else if(mode==="organisation")count=`${doc.querySelectorAll(".task").length} tâche(s)`;else if(mode==="heures")count=`${doc.querySelectorAll("tbody tr").length} ligne(s) affichée(s)`;live.textContent=count||"Fonctions d’origine conservées"}
function prepareFrame(){const doc=targetDoc();if(!doc)return;addTheme(doc,mode==="kontrol"?"kontrol-cloud":mode);if(mode==="kontrol"){const nested=doc.getElementById("kontrolFrame");if(nested){const inject=()=>addTheme(nestedKontrolDoc(),"kontrol");nested.addEventListener("load",()=>setTimeout(inject,60));setTimeout(inject,300);setTimeout(inject,1100)}}buildTools();mirrorStatus();setInterval(mirrorStatus,1200);loading.classList.add("hidden")}
