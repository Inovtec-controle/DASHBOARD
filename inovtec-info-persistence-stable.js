...
function applyExtras(d,site){
 const form=d.getElementById("siteForm");if(!form||!site)return;form.dataset.ivChantierId=String(site.id||form.dataset.ivChantierId||"");if(form.dataset.ivInfoDirty!=="1"){if(Array.isArray(site.agentsAffectes))form.dataset.ivAgentsAffectes=JSON.stringify([...new Set(site.agentsAffectes.map(v=>String(v||"").trim()).filter(Boolean))]);else form.dataset.ivAgentsAffectes="[]";if(Array.isArray(site.agentsAffectesNoms))form.dataset.ivAgentsAffectesNoms=JSON.stringify([...new Set(site.agentsAffectesNoms.map(v=>String(v||"").trim()).filter(Boolean))]);else form.dataset.ivAgentsAffectesNoms="[]";form.removeAttribute("data-iv-agents-draft");}
