...
function legalVisualRange(dates){const agentId=state.selected,h=weeklyHoursFor(agentId),amp=amplitudeLimitFor(agentId),keys=new Set(dates.map(dateInput));let starts=[];Object.keys(state.weeks||{}).forEach(w=>entriesForWeek(w).forEach(e=>{if(String(e.agentId)!==String(agentId))return;const d=dateFromWeek(w,Number(e.day)||0);if(keys.has(dateInput(d)))starts.push(timeToMin(e.start))}));if(!starts.length)Object.keys(state.weeks||{}).forEach(w=>entriesForWeek(w).forEach(e=>{if(String(e.agentId)===String(agentId))starts.push(timeToMin(e.start))}));let first=starts.filter(Number.isFinite).sort((a,b)=>a-b)[0];if(!Number.isFinite(first))first=6*60;const start=Math.max(0,Math.floor(first/60)*60),end=Math.max(start+60,Math.min(24*60,first+amp));const tip=document.querySelector(".agent-tip");if(tip){const info=h===null?"Heures contractuelles à renseigner dans le Classeur Agents":String(h).replace(".",",")+" h/semaine · amplitude de référence "+fmtDuration(amp);tip.dataset.legalInfo=info;tip.title=info}return{start,end}}
`;
