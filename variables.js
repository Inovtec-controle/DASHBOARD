...
function openModal(){leaveEditRevision=null;variableEditRevision=null;resetForm();$("variableModal").classList.add("open");$("variableModal").setAttribute("aria-hidden","false");setTimeout(()=>$("formType").focus(),30)}
...
function deleteCurrent(){
 ...
 if(source==="variables"&&String(state.entries.find(e=>String(e.id)===String(id))?.updatedAt||"")!==String(variableEditRevision||""))return alert("Cette variable a changé sur l’autre appareil. Rouvrez sa fiche.");
 closeModal();deleteRow({id,source,agentRefId:$("formAgent").value,updatedAt:variableEditRevision});
}
function cloneVariableState(value){return cleanState(parse(JSON.stringify(value||{}),{}))}
...
function maybeMigrateLegacyHours(){
  if(state.meta?.legacyHoursMigrated||!agents.length||!variablesReady||!legacyHoursResolved||!variablesCloudReady||!confirmedVariables||variableSaveInProgress)return;
...
function bindFirebase(){
  const f=fb();
  if(!f){variablesReady=false;legacyHoursResolved=false;syncNotice("Firebase indisponible : enregistrement désactivé.",true);return}
  f.auth().onAuthStateChanged(u=>{
    ...
    if(!user){docRef=null;confirmedVariables=null;$("loginBox").classList.add("open");variablesReady=false;legacyHoursResolved=false;return}
    ...
    unsub=docRef.onSnapshot({includeMetadataChanges:true},s=>{
      ...
      if(ve&&typeof ve.payload==="string"){
    ...
    },e=>{console.warn("Variables lecture Firebase",e);variablesCloudReady=false;variablesReady=false;legacyHoursResolved=false;syncNotice("Firebase inaccessible : enregistrement désactivé.",true)});
  });
}
